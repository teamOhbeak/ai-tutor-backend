  ...
  implements InterviewQuestionsService
{
  ...
  async submitAnswer(interviewId: number, answerRequestDto: AnswerRequestDto): Promise<AnswerResponse> {
    try {
      ...
      if (answerRequestDto.questionType === QuestionType.MAIN) {
        ...
        // 가장 높은 순서인거 구하기
      const maxSequence = await this.interviewQuestionsRepository.getHighestSequenceByQuestionId(answerRequestDto.interviewQuestionId);
      // 메인 질문 상태값 변경(status, updatedAt, sequence)
      const updatedQuestion = await this.interviewQuestionsRepository.updateQuestionStatus(answerRequestDto.interviewQuestionId,maxSequence);
      ...
      const updatedFollowUpQuestion = await this.followUpQuestionsRepository.updateQuestionStatus(answerRequestDto.followUpQuestionId);
