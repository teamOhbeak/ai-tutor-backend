...
@Injectable()
export class InterviewQuestionsServiceImpl implements InterviewQuestionsService {
  constructor(
    private readonly interviewQuestionsRepository: InterviewQuestionsRepositoryImpl,
  ) {}

  public async getQuestions(questionId: number, stack: string):Promise< allQuestionResponse[]>{
    console.log('서비스')

    try {
      console.log('레포가기전')

      const repo = await this.interviewQuestionsRepository.getQuestions(questionId, stack);
      console.log('레포간 후')
